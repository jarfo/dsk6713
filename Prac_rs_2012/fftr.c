...
PARAMETERS
	f:		 At the input (output for the IDFT), the array is of length 2*N and contains the real part of the input
			 signal on the odd-numbered elements, and the imaginary part on the even-numbered elements
			 At the output (input for the IDFT), it contains the samples of the DFT, ordered as follows:
				f[0]	Real(DFT(0))
				f[1]	Imag(DFT(0))
				f[2]	Real(DFT(1))
				f[3]	Imag(DFT(1))
				...
				f[2*N-2]	Real(DFT(N-1))
